...
# Adding a column indicating the year
```{r}
...
# Exporting Math_Oly_data file (For batch type conversion)
```{r}
write.csv(Math_Oly_data, "Math_Oly_data.csv", row.names = FALSE) 
```

# Inporting Math_Oly_data file (For batch type conversion)
```{r}
...
# Calculate the number of entries and average rankings for each country
```{r}
Math_Oly_data %>% 
  group_by(name) %>%
...
# import GDP_per_capita dataset
```{r}
GDP_per_capita <- read_csv("GDP_per_capita.csv")
```

# Combined with GDP_per_capita
```{r}
Math_Oly_data <- Math_Oly_data %>%
...
# Rebuild and re-analyze the data set.
# Importing GDP and education expenditure percentage data
```{r}
GDP_per_capita_v2 <- read_csv("GDP_per_capita_v2.csv")
Gov_expenditure_on_educa <- read_csv("Gov_expenditure_on_educa.csv")
```

# Extracting the necessary columns from Math Olympiad rank data
```{r}
Math_Oly_data2 <- Math_Oly_data[1:19]
```

# Combined with GDP_per_capita_v2
```{r}
...
# Check the number of lines with missing values
```{r}
